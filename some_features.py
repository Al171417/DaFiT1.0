        ...
        if st.checkbox("proceed", key=" model_proceed"):
            ...
            if len(y_var.columns) > 1:
                ...
                try:
                    ...
                    y_train2 = y_train.apply(pd.to_numeric, errors="coerce")

                    x_train2 = x_train2.fillna(x_train2.mean())
                    y_train2 = y_train2.fillna(y_train2.mean())

                    x_train2 = sm.add_constant(x_train2)
                    my_model = sm.OLS(y_train2, x_train2).fit()
                    summary_table = my_model.summary()
                except Exception as e:
                    try:
                        x_train2 = x_train.apply(pd.to_numeric, errors="coerce")
                        y_train2 = y_train.apply(pd.to_numeric, errors="coerce")
                        ...
                        x_train2 = np.asarray(x_train2, dtype=np.float64)
                        y_train2 = np.asarray(y_train2, dtype=np.float64)
